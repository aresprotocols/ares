...
fn test_account() {
	new_test_ext().execute_with(|| {
		...
		// let account_id: AccountId  = get_account_id_from_seed::<sr25519::Public>("Alice");
		// println!("account_id = {:?}", &account_id);
...
fn make_author_insert_key_params<'a >(raw_data: (&str, &str)) -> (&'a str, &'a str, String) {

	let mut account_id: Vec<u8> ;

	println!("raw_data.0 = {:?}", raw_data.0);
	if "grandpa" == raw_data.0 {
		println!(" RUN 1 ");
		account_id = ed25519::Pair::from_string(raw_data.1, None)
			.expect("Seed error of ed25519.").public().to_vec();
	}else{
		println!(" RUN 2 ");
		account_id = sr25519::Pair::from_string(raw_data.1, None)
	...
	let account_id = sp_core::hexdisplay::HexDisplay::from(&account_id);
	println!("account_id = {:?}", &account_id);
	("a","b",format!("0x{}", &account_id))
}

fn extract_hex_of_public<TPublic: Public>(raw_data: &str) -> HexDisplay {
	TPublic::Pair::from_string(raw_data).expect("Seed error")
}
