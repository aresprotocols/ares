...
use runtime_pioneer_node::{
	self, opaque::Block as PioneerBlock, part_ocw::LOCAL_STORAGE_PRICE_REQUEST_DOMAIN, RuntimeApi as PioneerRuntimeApi,
};
...
native_executor_instance!(
	pub PioneerExecutor,
	runtime_pioneer_node::api::dispatch,
	runtime_pioneer_node::native_version,
	frame_benchmarking::benchmarking::HostFunctions,
);

type FullClient = sc_service::TFullClient<PioneerBlock, PioneerRuntimeApi, PioneerExecutor>;
type FullBackend = sc_service::TFullBackend<PioneerBlock>;
type FullSelectChain = sc_consensus::LongestChain<FullBackend, PioneerBlock>;
...
pub fn new_partial(
	...
	sc_service::PartialComponents<
		...
		FullBackend,
		FullSelectChain,
		sc_consensus::DefaultImportQueue<PioneerBlock, FullClient>,
		sc_transaction_pool::FullPool<PioneerBlock, FullClient>,
		(
			sc_finality_grandpa::GrandpaBlockImport<
				...
				FullSelectChain,
			>,
			sc_finality_grandpa::LinkHalf<PioneerBlock, FullClient, FullSelectChain>,
			Option<Telemetry>,
		),
	>,
	ServiceError,
> {
	...
	let (client, backend, keystore_container, task_manager) =
		sc_service::new_full_parts::<PioneerBlock, PioneerRuntimeApi, PioneerExecutor>(
			&config,
...
pub fn new_full(
	...
	let rpc_extensions_builder = {
		...
		Box::new(move |deny_unsafe, _| {
			let deps =
				crate::rpc::FullDeps { client: client.clone(), pool: pool.clone(), deny_unsafe, backend: backend.clone() };
...
pub fn new_light(mut config: Configuration) -> Result<TaskManager, ServiceError> {
	...
	let (client, backend, keystore_container, mut task_manager, on_demand) =
		sc_service::new_light_parts::<PioneerBlock, PioneerRuntimeApi, PioneerExecutor>(
			&config,
			telemetry.as_ref().map(|(_, telemetry)| telemetry.handle()),
		)?;
