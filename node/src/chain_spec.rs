...
pub use ares_genesis::{GladiosAccountId, GladiosBalance, GladiosNodeChainSpec, GladiosSS58Prefix, GladiosWASM_BINARY};
use testnet_genesis::{get_account_id_from_seed, make_testnet_genesis};
pub use testnet_genesis::{PioneerAccountId, PioneerBalance, PioneerNodeChainSpec, PioneerSS58Prefix, PioneerWASM_BINARY};

// // For dev config.
// pub fn development_config() -> Result<PioneerNodeChainSpec, String> {
// 	let other_allocation_json = &include_bytes!("../res/ares.yml")[..];
// 	let chain_spec_config: ChainSpecConfig = serde_yaml::from_slice(other_allocation_json).unwrap();
...
// 				chain_spec_config.authorities.clone(),
// 				vec![],
// 				chain_spec_config.root.clone(),
// 				// Pre-funded accounts
// 				chain_spec_config.balances.clone(),
// 				chain_spec_config.council.clone(),
// 				true,
// 			)
// 		},
// 		boot_nodes.unwrap_or(vec![]),
// 		telemetry_endpoints.clone(),
// 		// Protocol ID
// 		None,
// 		// Properties
// 		None,
...
pub fn development_config() -> Result<PioneerNodeChainSpec, String> {
	...
	let immigration: Vec<(PioneerAccountId, PioneerBalance)> = serde_json::from_slice(chain_balance).unwrap();
	...
	Ok(PioneerNodeChainSpec::from_genesis(
		...
		move || {
			make_testnet_genesis(
				...
				Some(immigration.clone()),
				true,
			)
		},
		// Bootnodes
		vec!["/ip4/158.247.224.166/tcp/30333/p2p/12D3KooWEyoppNCUx8Yx66oV9fJnriXwCcXwDDUA2kj6vnc6iDEp"
			.parse()
			.unwrap()],
		// Telemetry
		None,
...
pub fn local_testnet_config() -> Result<PioneerNodeChainSpec, String> {
	...
	let initial_authorities: Vec<(
		...
		(
			...
			hex!["763a6ddd64b5e2f0e0c08a2c6e5143ae47edc563155bd052a26d3f942b806a1f"].unchecked_into(),
		...
		(
			...
			hex!["a483a387dd54aa61d1619bfca66b41e0bbee9cd199306e4310f823526d6ebe6a"].unchecked_into(),
	...
	let chain_balance = &include_bytes!("./chain_spec/gladios-balance.json")[..];
	let immigration: Vec<(GladiosAccountId, GladiosBalance)> = serde_json::from_slice(chain_balance).unwrap();
...
pub fn local_ares_config() -> Result<GladiosNodeChainSpec, String> {
	...
	Ok(GladiosNodeChainSpec::from_genesis(
		...
		// Bootnodes
		vec!["/ip4/45.77.243.246/tcp/30334/ws/p2p/12D3KooWMqDofvvwRtP7AYSAyJ2udNYrj698wnYJbpjomNTHHK9E"
			.parse()
			.unwrap()],
		// Telemetry
		None,
...
struct ChainSpecConfig {
	...
	symbols: Vec<(Vec<u8>, Vec<u8>, u32, u32, u8)>,
}

fn get_immigration_total_balance(immigration_account: Option<Vec<(AccountId, Balance)>>) -> Balance{
	let mut immigration_total_balance: Balance = Default::default();
	let mut immigration_balance:Vec<(AccountId, Balance)> = Default::default();
	if let Some(x) = immigration_account {
		let unique_allocation_accounts = x
			.iter()
			.map(|(account_id, amount)| {
				immigration_total_balance = immigration_total_balance.saturating_add(*amount);
				account_id
			})
			.cloned()
			.collect::<std::collections::BTreeSet<_>>();
		assert_eq!(unique_allocation_accounts.len(), x.len());
		immigration_balance = x.clone();
	}
	immigration_total_balance
}

pub struct AresBalanceConfig {
	pub total_issuance: Balance,
	pub per_election_desposit: Balance,
	pub per_staking_desposit: Balance,
	pub authorities_len: usize,
	pub council_len: usize,
	pub endowed_accounts: Vec<AccountId>,
	pub immigration_account: Option<Vec<(AccountId, Balance)>>,
}

pub fn merge_accounts_balance(
	balance_conf: AresBalanceConfig,
) -> Vec<(AccountId, Balance)> {

	let TOTAL_ISSUANCE: Balance = balance_conf.total_issuance.clone(); // one billion.
	let PER_ELECTION_DESPOSIT: Balance = balance_conf.per_election_desposit.clone();
	let PER_STAKING_DESPOSIT: Balance = balance_conf.per_staking_desposit.clone(); // Stake balance per validator.

	let authorities_len: usize = balance_conf.authorities_len.clone() ;
	let council_len: usize = balance_conf.council_len.clone() ;
	let endowed_accounts: Vec<AccountId>  = balance_conf.endowed_accounts.clone() ;
	let immigration_account: Option<Vec<(AccountId, Balance)>>  =  balance_conf.immigration_account.clone() ;

	let TOTAL_AUTHORITIES_ISSUANCE: Balance = PER_ELECTION_DESPOSIT.saturating_mul(council_len.try_into().unwrap_or(0));
	let TOTAL_COUNCIL_ISSUANCE: Balance = PER_STAKING_DESPOSIT.saturating_mul(authorities_len.try_into().unwrap_or(0));
	let mut immigration_total_balance = get_immigration_total_balance(immigration_account.clone());
	let REMAINING_BALANCE: Balance = TOTAL_ISSUANCE.saturating_sub(immigration_total_balance);

	assert!(REMAINING_BALANCE > TOTAL_AUTHORITIES_ISSUANCE.saturating_add(TOTAL_COUNCIL_ISSUANCE));
	let per_endowment: Balance = (REMAINING_BALANCE / endowed_accounts.len() as Balance).try_into().unwrap();
	assert!(per_endowment > PER_ELECTION_DESPOSIT);
	assert!(per_endowment > PER_STAKING_DESPOSIT);

	let endowed_accounts_for_balance: Vec<(AccountId, Balance)> = endowed_accounts.iter().cloned().map(|k| (k, per_endowment)).collect();
	let balance_accounts = [endowed_accounts_for_balance, immigration_account.unwrap_or(vec![])].concat();

	// Check for duplicates
	let unique_allocation_accounts = balance_accounts
		.iter()
		.map(|(account_id, amount)| {
			immigration_total_balance = immigration_total_balance.saturating_add(*amount);
			account_id
		}).cloned().collect::<std::collections::BTreeSet<_>>();
	assert_eq!(unique_allocation_accounts.len(), balance_accounts.len());
